...
use crate::export::{Block, Book, Piece, blocks, chapter_heading, inline, pack, xml_escape};
    ...
    for piece in &b.pieces {
        match piece {
            ...
            Piece::Chapter {
                ...
                let mut paras = Vec::new();
                sink(layout, &mut paras);
                // The same headings as the manuscript: a Prologue keeps its
                // own, numbered chapters follow the chosen style.
                let (heading, name) = chapter_heading(*number, title, b.look.chapter_heading);
                paras.push(Para::text(Style::ChapterNumber, &heading));
...
fn soffice() -> Option<(Vec<String>, bool)> {
    // One way of finding LibreOffice for every PDF (see [`crate::pdf`]).
    match crate::pdf::find()? {
        crate::pdf::Office::Binary(p) => Some((vec![p.to_string_lossy().to_string()], false)),
        crate::pdf::Office::Flatpak => Some((
            vec![
                "flatpak".into(),
                "run".into(),
