...
pub struct Exported {
    ...
    pub pages: usize,
    /// Anything worth saying about what was (or wasn't) written, e.g. that a
...
pub fn export(p: &Project, opts: &ExportOptions) -> Result<Exported> {
    p.ensure_whole()?;
    if !(opts.docx || opts.epub || opts.markdown || opts.paperback) {
        bail!("nothing to export — choose DOCX, EPUB, Paperback or Markdown");
    }
    ...
    if opts.epub {
        ...
        put("epub", &epub_with(&book, &extras)?)?;
    ...
    if opts.paperback {
        ...
        let docx_path = dir.join(format!("{stem}-paperback.docx"));
        crate::atomic::write(&docx_path, &docx_bytes)?;
        files.push(docx_path.clone());
        match crate::export_print::to_pdf(&document, &docx_path.with_extension("pdf")) {
